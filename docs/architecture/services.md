...
The reviewed mock error had only `error.code` and `error.message` with `SAVE_FAILED`; that is not carried forward because the merged service contract already has a richer project-wide error catalog. The frontend adapter must map retryable API errors to the existing non-technical UI text, "We could not save that change. Please try again.", rather than branching on mock-only `SAVE_FAILED`.
...
|---|---|---|---|
| `data` | array of todo objects | no | Current todos. Empty array means no todos exist. |
| `data[].id` | string UUID | no | Stable todo identifier. |
| `data[].title` | string | no | Trimmed todo title, 1 to 200 characters. |
| `data[].status` | string enum | no | `active` or `completed`. |
| `data[].createdAt` | string timestamp | no | Creation time, RFC 3339 UTC. |
| `data[].updatedAt` | string timestamp | no | Last successful update time, RFC 3339 UTC. |
| `meta.total` | integer | no | Count of returned todos. |
| `meta.active` | integer | no | Count of returned todos with `status = active`. |
| `meta.completed` | integer | no | Count of returned todos with `status = completed`. |
...
|---|---|---|---|
| 1 | No additional database migration beyond the existing `todos` table design. | n/a | Safe; no schema change. |
| 2 | Backend maps `todos.is_completed` to API `status` and computes `meta` counts from the returned collection. | Revert adapter mapping to the prior mock-only frontend state if the backend integration is rolled back. | Safe; application-code-only change. |
...
|---|---|---|---|
| Todo response envelope | Use reviewed mock shape `{ data, meta }`. | Keep the earlier generic `{ todos, next_cursor, has_more }` envelope for this story. | The approved UI is already built against `{ data, meta }`; the SRS only requires rendering 100 todos, so pagination fields would cause unnecessary UI rework now. |
