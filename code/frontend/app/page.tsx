...
export default function HomePage() {
  ...
  async function load() { setLoading(true); setError(""); try { const res = await request<{data: Todo[]}>("/todos"); setTodos(res.data); } catch { setError("We could not load your tasks. Please try again."); } finally { setLoading(false); } }
  useEffect(() => { void load(); }, []);

  async function add(e: FormEvent) { e.preventDefault(); const trimmed = title.trim(); if (!trimmed) { setValidation("Task cannot be blank."); return; } setValidation(""); setError(""); try { const todo = await request<Todo>("/todos", { method: "POST", body: JSON.stringify({ title: trimmed }) }); setTodos((items) => [...items, todo]); setTitle(""); } catch { setError(SAVE_ERROR); } }
  async function toggle(todo: Todo) { const next = todo.status === "active" ? "completed" : "active"; setTodos((items) => items.map((item) => item.id === todo.id ? { ...item, status: next } : item)); setError(""); try { const saved = await request<Todo>(`/todos/${todo.id}`, { method: "PATCH", body: JSON.stringify({ status: next }) }); setTodos((items) => items.map((item) => item.id === todo.id ? saved : item)); } catch { setTodos((items) => items.map((item) => item.id === todo.id ? todo : item)); setError(SAVE_ERROR); } }
  async function remove(todo: Todo) { setTodos((items) => items.filter((item) => item.id !== todo.id)); setError(""); try { await request<void>(`/todos/${todo.id}`, { method: "DELETE", headers: {} }); } catch { setTodos((items) => items.some((item) => item.id === todo.id) ? items : [...items, todo]); setError(SAVE_ERROR); } }
